//TODO: connect from websocket no matter csp (try it but doe snot enable to open content html anymore
// From now open console only work if csp authorized it
// https://discourse.mozilla.org/t/content-scripts-and-websocket/27814/9
let actions = [];
let newtaburl = "";
...
const clearActions = () => {
	getCurrentTab().then((response) => {
		...
		actions = [
			...
			{title:"Help", desc:"Get help with console on GitHub", type:"action", action:"url", url:"https://github.com/alyssaxuu/console", emoji:true, emojiChar:"🤔", keycheck:false},
...
browser.runtime.onInstalled.addListener((object) => {
  ...
  if (object.reason === "install") {
    browser.tabs.create({ url: "https://github.com/ariary/console.sh" });
  }
});

// Check when the extension button is clicked
browser.browserAction.onClicked.addListener((tab) => {
	browser.tabs.sendMessage(tab.id, {request: "open-console"});
});

// Listen for the open console shortcut
browser.commands.onCommand.addListener((command) => {
	if (command === "open-console") {
		getCurrentTab().then((response) => {
			if (!response.url.includes("browser://") && !response.url.includes("browser.google.com")) {
				browser.tabs.sendMessage(response.id, {request: "open-console"});
			} else {
				browser.tabs.create({
					url: "./newtab.html" 
				}).then(() => {
...
// Restore the new tab page (workaround to show console in new tab page)
...
browser.tabs.onUpdated.addListener((tabId, changeInfo, tab) => resetconsole());
browser.tabs.onCreated.addListener((tab) => resetconsole());
browser.tabs.onRemoved.addListener((tabId, changeInfo) => resetconsole());
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
	switch (message.request) {
		...
		case "close-console":
			getCurrentTab().then((response) => {
				browser.tabs.sendMessage(response.id, {request: "close-console"});
			});
			break;
		}
});
